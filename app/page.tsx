...
export default function ChatBot() {
  ...
  const guestModeHook = useGuestMode()
  const guestSession = guestModeHook.session
  ...
  const [showBugBounty, setShowBugBounty] = useState(false) // Adding bug bounty modal state
  ...
  const { channel, hasChannel, createChannel, allChannels, refreshAllChannels } = useUserChannel(userId, null)
  ...
  const handleKeyPressWithGuest = (e: React.KeyboardEvent) => {
  ...
  useEffect(() => {
    ...
    if (refCode) {
      ...
      localStorage.setItem("p314_referral_code", refCode)
  ...
  const handleExitGuestMode = () => {
    ...
    guestModeHook.exitGuestMode()
  }

  if (!isSessionValid && !isGuestMode && (needsAuth || !sessionData)) {
    return (
      <LoginPage
  ...
  return (
    ...
    >
      {isGuestMode && guestSession && (
        <GuestModeBanner session={guestSession} onExitGuest={handleExitGuestMode} />
      )}

      <div
        className="flex items-center justify-center flex-1 p-2 sm:p-4"
        style={{ backgroundColor: COLORS.BACKGROUND }}
      >
        ...
        <CardHeader className="text-white rounded-t-lg p-3 sm:p-6" style={{ backgroundColor: COLORS.PRIMARY }}>
          <div className="flex items-center justify-between gap-3 sm:gap-4 mb-2">
            {/* Left Section: Share & Notifications */}
        ...
        <CardFooter className="p-2 sm:p-4 border-t">
          <div className="flex w-full gap-1.5 sm:gap-2 relative">
            ...
            <Input
              ...
              onKeyDown={handleKeyPressWithGuest}
              disabled={isLoading}
      ...
      <FraudReportModal
        ...
        username={username}
        piAccessToken={piAccessToken}
        onWalletVerify={(wallet) => {
          setPrefilledWallet(wallet)
          setShowWalletSearch(true)
        }}
      />
      ...
      <ChannelListModal
        ...
        onRefresh={refreshAllChannels}
        showOnlyJoined={false}
      />

      <ChannelListModal
        isOpen={showMyChannels}
        onClose={() => setShowMyChannels(false)}
        channels={allChannels}
        joinedChannels={joinedChannels}
        onJoinChannel={handleJoinChannel}
        onRefresh={refreshAllChannels}
